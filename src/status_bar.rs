...
pub fn create_control(instance: &HINSTANCE, parent: HWND) -> Result<HWND> {
    ...
    let hwnd = unsafe {
        CreateWindowExW(
            ...
            0,
            Some(parent),
            Some(HMENU(crate::resources::ID_STATUS_BAR as *mut c_void)),
            Some(*instance),
            None,
        )?
    };
    ...
    unsafe {
        SendMessageW(
            hwnd,
            SB_SETPARTS,
            Some(WPARAM(STATUS_BAR_NUM_PARTS)),
            Some(LPARAM(&raw mut parts as isize)),
        );
        Ok(hwnd)
    }
}
